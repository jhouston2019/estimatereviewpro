 ...
 * Shared by Netlify compare-estimates and the upload wizard client.
 */
...
function moneyMatchesOnLine(working) {
  const matches = [
    ...working.matchAll(
  ...
  return [
    ...working.matchAll(/\$\s*([\d]{1,3}(?:,\d{3})*\.\d{2})/g),
  ];
}
...
function extractDollarLineItems(text) {
  ...
  for (const raw of lines) {
    ...
    const moneyMatches = moneyMatchesOnLine(working);
...
function flagRow(carrierAmount, contractorAmount) {
  ...
  if (
    carrierAmount > 0 &&
    Math.abs(contractorAmount - carrierAmount) > 0.25 * carrierAmount
  ) {
    return true;
  }
  return false;
