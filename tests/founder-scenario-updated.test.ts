...
export async function runFounderScenario() {
  ...
  try {
    ...
    // ========================================
    // STEP 4: Calculate Deviations (ROOM-AWARE TRUE GEOMETRY)
    // ========================================
    console.log('\n[4/5] Calculating deviations with ROOM-AWARE TRUE GEOMETRY...');
    
    const deviations = calculateRoomAwareDeviations(
      structuredEstimate,
      expertReport,
      expectedQuantities
    );
    ...
    // Check 5: Geometry details present in audit trail
    if (deviations.auditTrail.geometryCalculations.length > 0) {
      console.log(`  ✓ Geometry calculations logged in audit trail (${deviations.auditTrail.geometryCalculations.length} calculations)`);
      results.checks.geometryAuditTrail = true;
      
      // Log sample geometry
      const sample = deviations.auditTrail.geometryCalculations[0];
      console.log(`    Sample: Perimeter ${sample.perimeter.toFixed(0)} LF, Wall Height ${sample.wallHeight} ft`);
      console.log(`    Formula: ${sample.formula}`);
    } else {
      console.log(`  ✗ No geometry calculations in audit trail`);
      results.errors.push('Geometry calculations not logged');
    }
    
    // Check 6: Ceiling height from dimensions (not hardcoded 8ft)
    if (deviations.auditTrail.avgCeilingHeight === 8) {
      console.log(`  ✓ Average ceiling height: ${deviations.auditTrail.avgCeilingHeight} ft (from dimensions)`);
      results.checks.ceilingHeightFromDimensions = true;
    } else {
      console.log(`  ⚠ Average ceiling height: ${deviations.auditTrail.avgCeilingHeight} ft`);
