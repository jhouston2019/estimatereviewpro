-- Allow 'premier' on users.plan_type and teams.plan_type
...
ALTER TABLE public.users
  ...
  CHECK (
    plan_type IS NULL
    OR plan_type IN ('single', 'professional', 'enterprise', 'premier')
  );
...
ALTER TABLE public.teams
  ...
  CHECK (plan_type IN ('professional', 'enterprise', 'premier'));
